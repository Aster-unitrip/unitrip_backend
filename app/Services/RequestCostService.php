...
class RequestCostService
{
    public function after_delete_component_cost($itinerary_group_accounting, $itinerary_group_cost, $delete_component){
        ...
        if($delete_component['type'] === "attractions" || $delete_component['type'] === "activities"){
            ...
            for($i = 0; $i < count($delete_component["pricing_detail"]); $i++){
                if($delete_component['pricing_detail'][$i]['name'] === "全票"){
                    $delete_component_data["accounting.0.adult.0.cost"] = $itinerary_group_accounting[0]["adult"][0]["cost"] - $delete_component['pricing_detail'][$i]['unit_price'];
                }
                elseif($delete_component['pricing_detail'][$i]['name'] === "半票"){
                    $delete_component_data['accounting.0.child.0.cost'] = $itinerary_group_accounting[0]["child"][0]["cost"] - $delete_component['pricing_detail'][$i]['unit_price'];
                }
            }
        }
        ...
        if($delete_component['type'] === "accomendations" || $delete_component['type'] === "restaurants" || $delete_component['type'] === "transportations"){
            ...
            $delete_component_data['accounting.0.child.0.cost'] = $itinerary_group_accounting[0]["child"][0]["cost"] - floor($cost_all / $count_all);
            $delete_component_data['accounting.0.adult.0.cost'] = $itinerary_group_accounting[0]["adult"][0]["cost"] - floor($cost_all / $count_all);
        }
        ...
        if($delete_component['type'] === "guides"){
            ...
            $delete_component_data['accounting.0.child.0.cost'] = $itinerary_group_accounting[0]["child"][0]["cost"] - floor($delete_component["subtotal"] / $delete_component['total_people']);
            $delete_component_data['accounting.0.adult.0.cost'] = $itinerary_group_accounting[0]["adult"][0]["cost"] - floor($delete_component["subtotal"] / $delete_component['total_people']);
        }
        // Done (目前不需要) 回推小孩大人成本error

/*         if($delete_component_data['accounting.0.child.0.cost'] < 0 || $delete_component_data['accounting.0.adult.0.cost'] < 0){
            return response()->json(["扣除後，小孩成本或大人成本不可小於0"], 400);
        } */
