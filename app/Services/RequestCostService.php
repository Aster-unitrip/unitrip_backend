...
{
    public function after_delete_component_cost($itinerary_group_accounting, $itinerary_group_cost, $delete_component, $total_people){
        ...
        if($delete_component['type'] === "attractions" || $delete_component['type'] === "activities"){
            ...
            for($i = 0; $i < count($delete_component["pricing_detail"]); $i++){
                ...
                else if($delete_component['pricing_detail'][$i]['name'] === "半票"){
        ...
        else if($delete_component['type'] === "accomendations" || $delete_component['type'] === "restaurants" || $delete_component['type'] === "transportations"){
            $delete_component_data['itinerary_group_cost'] = $itinerary_group_cost - $delete_component['sum'];
            if($delete_component_data['itinerary_group_cost'] < 0){
                return response()->json(["扣除後的供應商該項目的 itinerary_group_cost 不可小於0"], 400);
            }
            $delete_component_data['accounting.child.cost'] = ceil($delete_component['sum'] / $total_people);
            $delete_component_data['accounting.adult.cost'] = ceil($delete_component['sum'] / $total_people);
        }
        else if($delete_component['type'] === "guides"){
            $delete_component_data['itinerary_group_cost'] =  $itinerary_group_cost - $delete_component['subtotal'];
            if($delete_component_data['itinerary_group_cost'] < 0){
                return response()->json(["扣除後的供應商該項目的 itinerary_group_cost 不可小於0"], 400);
            }
            $delete_component_data['accounting.child.cost'] = ceil($delete_component["subtotal"] / $total_people);
            $delete_component_data['accounting.adult.cost'] = ceil($delete_component["subtotal"] / $total_people);
        }
